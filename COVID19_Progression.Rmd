...
The current Australian numbers used in all plots below **`r ifelse(updateAU, "do", "do not")` contain** any cases reported since the JHU dataset was updated.
As such, the latest values for infection rate *`r ifelse(updateAU, "may be incomplete for today as all states may not yet have reported", "may lag official figures by a few hours")`*.
...
## Currently Active Infections Rates
...
cpFr <- glue(
  ...
  Only countries with {minDays} days of data beyond this time-point are shown, and are shown in order of the date at which they passed this threshold.
  Countries with no recorded fatalities have been excluded. 
  A clear trend of an increasing fatility rate with time is evident in many countries (e.g. Spain, France, Italy, UK), however for some countries at the bottom of this plot (e.g. Singapore), this rate appears relatively stable throughout the majority of days.
