...
```{r alwaysShow}
alwaysShow <- c("Australia", "New Zealand", "US", "United Kingdom", "Taiwan*", "Singapore", "South Korea", "China (Other)", "China (Hubei)", "Hong Kong", "Japan", "Ireland", "Russia", "Brazil", "Kuwait", "Finland", "Norway")
```
...
**As of June 6th, Brazil has stopped releasing any figures related to COVID-19. All values related to Brazil will be extremely outdated and inaccurate.**
...
```{r plotRegionDailyCases, fig.height=8, fig.width=10, fig.cap = "*Daily new confirmed infections, shown as cases / million. Values are calculated within the total population for each continent. The solid black line is the 7-day exponential moving average calculated within each continent*"}
ggplotly(
  confirmed %>% 
    inner_join(wikiPops) %>%
    mutate(
...
```{r plotNewDailyCountry, fig.width=10, fig.height=15, fig.cap = "*Daily New Confirmed Cases (per million) using each country's population to calculate the case rate.*"}
minPop <- 10e6
ggplotly(
  confirmed %>%
    group_by(Country, date) %>%
    ...
    mutate(
      `Daily Cases` = round(1e6 * daily / Population, 2)
    ) %>%
    mutate(
      Region = str_replace_all(
        Region, "(Southern|Western) Europe", "Southern & Western Europe"
    ...
    rename_all(str_to_title) %>%
    ggplot(
      aes(Date, `Daily Cases`, fill = Country)
    ) +
    geom_bar(stat = "identity") +
    facet_wrap(
      ~Region, ncol = 2, scales = "free_y"
    ) +
    labs(
...
ggplotly(plotIncVConf +
  coord_cartesian(
    ylim = c(0.3, max(plotIncVConf$data$`Average Daily Increase`)))
)
```
...
ggplotly(
  dailyVsTotalDeaths +
    coord_cartesian(
      ylim = c(0.01, max(dailyVsTotalDeaths$data$`Daily Fatalities`))
    )
)
```
...
cp2 <- glue(
    ...
    __Recovered cases as poorly and irregularly reported by many countries (e.g. Ireland & Norway).
