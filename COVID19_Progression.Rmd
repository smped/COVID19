...
**As of June 6th, Brazil has also stopped releasing any offical figures related to COVID-19.**
...
The current `r n` day growth factor is `dplyr::filter(gf$data, date == max(date), confirmed == max(confirmed))$R` which gives `r ifelse(dplyr::filter(gf$data, date == max(date), confirmed == max(confirmed))$R > 1, "*considerable cause for concern*", "some degree of confidence that the spread of infections is relatively under control")`.
