...
```{r boxplotRates, fig.height=7, fig.width=8, fig.cap = "*Infection rates scaled by population showing the influence of governing party. The p-values from a simple t-test is provided in the plot window.*"}
tTest <- df %>% 
...
df %>% 
  ...
  mutate(
    ...
    Incident_Rate = 0.1*Incident_Rate
  ) %>%
  ggplot(
    aes(Party, Incident_Rate, fill = Party)
  ) +
  geom_boxplot() +
  geom_text(
    aes(x, Incident_Rate, label = label),
    data = . %>%
      dplyr::filter(
        Incident_Rate == max(Incident_Rate)
      ) %>%
      mutate(
        x = 1,
        label = paste("p =", round(tTest$p.value, 3))
      ),
    inherit.aes = FALSE
  ) +
  scale_fill_manual(values = c(Democrat = "blue", Republican = "red")) +
  labs(y = "Confirmed Infections / Million")
```
