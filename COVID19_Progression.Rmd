...
nswTable <- nswUrl %>% 
  ...
  mutate(Count = str_remove_all(Count, "[\\*,]") %>% as.numeric)
nswTests <- nswTable %>%
  dplyr::filter(
...
latestAU$WA <- tibble(
  ...
  date = dt,
  confirmed = waTxt %>% 
    str_subset("(cases|total).+(total|cases)") %>% 
...
cp <- glue(
  ...
  Importantly, whilst the fatality count in countries like the US and the UK are below the statistical value of 'excess deaths' observed throughout these countries, this is not true for Belgium and the fataility rate for Belgium is more likely to reflect an accurate assessment of the true fatalities due to COVID-19.
...
This visualisation **fails to account for imported cases** and assumes that all cases were transmitted within each state.
Recent patterns in both WA and SA have shown many cases are still being imported from overseas.
