...
if (url.exists(qldUrl)){
  qldTests <-qldUrl %>%
    read_html() %>%
    html_nodes("body") %>%
    xml_find_all("//td[contains(@headers, 'table59454r1c2')]") %>%
    html_text() %>%
    .[[1]] %>%
    str_remove_all(",") %>%
    as.numeric()
...
latestAU <- fromJSON("https://api.infotorch.org/api/covid19/totals/") %>%
  ...
  left_join(
    here::here(
      glue("recovered/recovered_{as.character(dt)}.tsv")
    ) %>%
      read_tsv()
  )
...
a <- deaths %>%
  ...
  geom_label(
    aes(y = rate+8, label = round(rate, 0)),
...
b <- deaths %>%
  ...
  geom_col(colour = "black") +
  geom_label(
    aes(y = deaths + 800, label = comma(deaths, accuracy = 1)),
    fill = "white", alpha = 0.5
  ) +
  scale_fill_viridis_d(option = "magma", direction = 1) +
  scale_y_continuous(expand = expand_scale(c(0, 0.05)), labels = comma) +
