...
nswTables <- nswUrl %>%
  ...
  .[1:2] %>%
  lapply(function(x){
    names(x) <- str_extract(names(x), "[A-Za-z]+")
    x
  })
nswTests <- nswTables[[1]] %>%
  dplyr::filter(
  ...
  str_remove_all(",") %>%
  as.numeric()
nswRecov <- nswTables[[2]] %>%
  dplyr::filter(Outcome == "Recovered") %>%
  .[["Number"]] %>%
  as.numeric()
```
...
tasTables <- tasUrl %>%
  ...
  xml_find_all("//table") %>%
  html_table()
tasRecov <- tasTables[[1]] %>%
  dplyr::filter(`Cases in Tasmania` == "Recovered") %>%
  .[["Number"]] %>%
  as.numeric()
tasTests <- tasTables[[2]] %>%
  dplyr::filter(`Laboratory tests` == "Total laboratory tests") %>%
  .[["Number"]] %>%
  str_remove_all("[, ]") %>%
  as.numeric()
```
...
p <- confirmed %>%
  ...
  ungroup() %>%
  inner_join(
    dplyr::filter(wikiPops, Population > minPop | Country %in% c("Singapore", "Hong Kong"))
  ) %>%
  mutate(
    rate = 1e6*confirmed / Population
  ) %>%
...
Differences in the State-level intercepts are not particularly relevant, apart from capturing the initial time at which cumulative confirmed infection rate exceeded `r minRate` cases / million and these are not shown.
