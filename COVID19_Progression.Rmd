...
As such, the latest values for infection rate *`r ifelse(updateAU, "may be __incomplete for today__ as all states may not yet have reported", "may lag official figures by a few hours")`*.
`r ifelse(updateAU, glue("{n} states/territories have currently reported updated figures since the overnight JHU release."), "")`
...
pops <- tribble(
    ...
    "China (Hubei)", hb, "Asia",
    "China (Other)", 1408526449 - hb - hk, "Asia",
...
- Only countries are shown where the infection rate has surpassed `r startingPoint` cases/million. This Cumulative Confirmed Infection Rate equates to about `r round(startingPoint*dplyr::filter(pops, Country == "Australia")$Population / 1e6,0)` confirmed cases within Australia, and is comparable to the days since passing 100 confirmed cases commonly shown elsewhere.
...
- Overall, the Australian infection rate brings mild relief that we are doing a little better than many other countries. However testing rates are far lower than in countries like South Korea and many genuine cases are likely to be undetected and unreported
...
cpFr <- glue(
  ...
  A clear trend of an increasing fatality rate with time is evident in many countries (e.g. Spain, France, Italy, UK), however for some countries at the bottom of this plot (e.g. Singapore), this rate appears relatively stable throughout the majority of days.
...
Testing numbers were originally sourced from https://covid-19-au.github.io/, then were automatically updated should any later figures be provided at https://api.infotorch.org/api/covid19/totals/.
