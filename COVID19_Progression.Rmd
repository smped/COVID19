...
nswTests <- nswUrl %>%
  read_html() %>%
  html_nodes("body")  %>%
  xml_find_all("//table[contains(@class, 'moh-rteTable-6')]") %>%
  html_text() %>%
  .[[1]] %>%
  str_split("\r\n") %>%
  .[[1]] %>%
  .[which(str_detect(., "Total cases tested$")) + 1] %>%
  str_extract("^[0-9,]+") %>%
  str_remove(",") %>%
  as.numeric()
```
...
```{r qldTests}
qldTests <- NA_real_
qldUrl <- "https://www.qld.gov.au/health/conditions/health-alerts/coronavirus-covid-19/current-status/current-status-and-contact-tracing-alerts"
...
# Might need to revert to the dated url
# vicUrl <- "https://www.dhhs.vic.gov.au/coronavirus-covid-19-daily-update"
vicUrl <- glue(
  "https://www.dhhs.vic.gov.au/coronavirus-update-victoria-{format(dt, '%d-%B-%Y')}"
) %>%
  str_to_lower()
...
saTxt <- saUrl  %>%
  ...
  .[. != ""]
saTests <- saTxt %>%
  str_replace_all(".+ ([0-9, ]+) COVID-19 laboratory tests.+", "\\1") %>%
  str_remove_all("[, ]") %>%
  as.numeric()
saRecov <- saTxt %>%
...
tasTests <- NA_real_
tasUrl <- glue(
  "http://health.tas.gov.au/news/2020/coronavirus_update_{str_remove(format(dt, '%d_%B_%Y'), '^0')}"
) %>%
  str_to_lower()
...
if (nrow(auRec) > 0 ){
  auRec %>%
    write_tsv(
      here::here(glue("recovered/recovered_{ymd(Sys.Date())}.tsv"))
    )
}
```
...
`r ifelse(updateAU, glue("As of {format(Sys.time(), '%H:%M')}, __{n} of 8 states/territories have released updated COVID-19 figures since the overnight JHU release.__"), "")`
...
a <- deaths %>%
  ...
  geom_label(
    aes(y = rate + 18, label = round(rate, 0)),
    fill = "white", alpha = 0.5
  ) +
  scale_fill_viridis_d(option = "magma", direction = 1) +
  scale_y_continuous(expand = expand_scale(c(0, 0.08))) +
...
b <- deaths %>%
  ...
  geom_col(colour = "black") +
  geom_label(
    aes(y = deaths + 1800, label = comma(deaths, accuracy = 1)),
    fill = "white", alpha = 0.5
  ) +
  scale_fill_viridis_d(option = "magma", direction = 1) +
  scale_y_continuous(expand = expand_scale(c(0, 0.08)), labels = comma) +
...
deaths %>%
  ...
  datatable(
    options = list(
      ...
      searchCols = list(
        ...
        list(
          search = glue(
            '{minCases + 0.1} ... {max(.$`Confirmed Cases`)}'
          )
        ),
        NULL, NULL, NULL
      )
    ...
    rownames = FALSE,
    caption = glue(
      "All countries with recorded fatalities, sorted by default in order of the Fatality Rate.
  ...
  formatRound(
    columns = c("Confirmed Cases", "Fatalities"),
    mark = ",", 
    digits = 0
  ) %>%
  formatRound(
...
- It is now clear that Australia is **no longer in an exponential growth phase** and there are **clear signs of the growth levelling out**. The contribution of the distortions caused by the Ruby Princess to this apparent levelling out is still unclear
...
- Notably, the proportion of recovered cases is now >50%
...
confirmed %>% 
  ...
  bind_rows(
    group_by(., date) %>%
      summarise(
        ...
        FRate = sum(deaths, na.rm = TRUE) / sum(confirmed),
        d = sum(d),
        p = d / (confirmed - d)
      ) %>%
      ungroup() %>%
  ...
  mutate(
    RRate = case_when(
      `Province/State` != "Total" ~ RRate,
      `Province/State` == "Total" ~ sum(recovered[`Province/State` != "Total"], na.rm = TRUE) / sum(confirmed[!is.na(recovered) & `Province/State` != "Total"])
    )
  ) %>% 
  pivot_wider(
  ...
  dplyr::select(State = `Province/State`, 2:3, ends_with("Change"), starts_with("Fatal"), starts_with("Recov")) %>%
  mutate(
     `Currently Active` = (!!as.name(as.character(dt))) - Fatalities - `Recovered^+^`
  ) %>%
  pander(
    justify = "lrrrrrrrrr",
    caption = paste(
      ...
      "^+^Recovered cases are not reported by NSW so this value is taken by subtracting all other states from the national total.*"
    ),
...
```{r plotStates, fig.width=10, fig.height=8, fig.cap="*Breakdown of individual states. Victorian recovered numbers began to be accurately reported from 22^nd^ March, with other states gradually providing this information. NSW/QLD recovered cases are still poorly reported and these estimates are unreliable.*"}
ggplotly(
  confirmed %>%
    dplyr::filter(Country == "Australia") %>%
    left_join(deaths) %>%
...
Taking only the last 2 weeks of data, **the infection rate in most states is doubling every `r round(log(2, base = 10^coef(lm)["date"]), 1)` days.**
