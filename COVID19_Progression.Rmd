...
vicUrl <- c(
  ...
  "https://www.dhhs.vic.gov.au/coronavirus-update-victorians-{day(dt)}-{format.Date(dt, '%B')}-{year(dt)}",
  "https://www.dhhs.vic.gov.au/coronavirus-update-victoria-{format(dt - 1, '%d')}-{format.Date(dt - 1, '%B')}-{year(dt-1)}",
    "https://www.dhhs.vic.gov.au/coronavirus-update-victorians-{day(dt - 1)}-{format.Date(dt - 1, '%B')}-{year(dt-1)}"
  ) %>% 
...
latestAU$VIC <- tibble(
  ...
  confirmed = vicTxt %>%
    ...
    str_replace_all(".+[^0-9,]+is ([0-9,]+)[^0-9,].+", "\\1") %>% 
    str_remove_all(",") %>%
    as.numeric() %>%
    .[!is.na(.)] %>%
    unique(),
...
**As of June 6th, Brazil has also stopped releasing any official figures related to COVID-19.**
...
These countries are `r pander(sort(alwaysShow))`
...
```{r, fig.width=10, fig.height=10, fig.cap=glue("*Most impacted 25 countries when combining rankings across both total confirmed cases, and total confirmed cases/million. Only countries with a population greater than {comma(minPop)} are shown. At the time of preparation, the total number of global infections stands at {comma(totalConf)}.*")}
confirmed %>%
  group_by(Country) %>%
  dplyr::filter(date == max(date)) %>%
  group_by(Country, date) %>%
...
- Notably, the proportion of recovered cases is now >90%
- NSW Health made a significnt change to their recovered cases on `r format.Date(min(nswJSON$date), "%d, %B %Y")` when **cases not tracked after 6 weeks were classified as recovered**
...
```{r plotStates, fig.width=10, fig.height=8, fig.cap="*Breakdown of individual states. Victorian recovered numbers began to be accurately reported from 22^nd^ March, with other states gradually providing this information. NSW/QLD recovered cases have only recently begun being reported and up until the most recent dates, recovered/active values were very approximate for these states. The extreme drop for NSW active cases in early June is a function of teh changed reporting strategy implemented by NSW Health.*"}
ggplotly(
  confirmed %>%
    dplyr::filter(Country == "Australia") %>%
    left_join(deaths) %>%
...
```{r, fig.width=10, fig.height=8, fig.cap="*Daily new cases for each state shown against the 7-day average. Days which are above average are highlighted in red.*"}
ggplotly(
  confirmed %>%
    dplyr::filter(Country == "Australia") %>%
    group_by(`Province/State`) %>%
