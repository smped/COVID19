...
- __The current fatality rate is `r confirmed %>% left_join(recovered) %>% left_join(deaths) %>% group_by(Country, date) %>% summarise_at(vars(confirmed, recovered, deaths), sum) %>% dplyr::filter(date == max(date)) %>% ungroup() %>% summarise(f = sum(deaths)/sum(confirmed)) %>% .[["f"]] %>% percent(accuracy = 0.1)`.__
This may be a function of under-reporting of true cases and is likely to be an overestimate.
- __The current recovery rate is `r confirmed %>% left_join(recovered) %>% left_join(deaths) %>% group_by(Country, date) %>% summarise_at(vars(confirmed, recovered, deaths), sum) %>% dplyr::filter(date == max(date)) %>% ungroup() %>% summarise(r = sum(recovered)/sum(confirmed)) %>% .[["r"]] %>% percent(accuracy = 0.1)`.__
Given the level of under-reporting this may also be highly inaccurate.
- At this point, __`r confirmed %>% left_join(recovered) %>% left_join(deaths) %>% group_by(Country, date) %>% summarise_at(vars(confirmed, recovered, deaths), sum) %>% dplyr::filter(date == max(date)) %>% ungroup() %>% summarise(r = (sum(confirmed) - sum(recovered) - sum(deaths)) / sum(confirmed)) %>% .[["r"]] %>% percent(accuracy = 0.1)` of all confirmed infections are considered as 'active'.__
