...
wikiPops <- "https://en.wikipedia.org/wiki/List_of_countries_by_population_(United_Nations)" %>%
  read_html() %>%
  html_nodes("body") %>%
  xml2::xml_find_all("//table[contains(@class, 'wikitable')]") %>% 
  html_text() %>%
  .[[2]] %>%
  str_split(pattern = "\n") %>%
  .[[1]] %>%
...
confirmed %>%
  ...
  mutate(
    lab_y = case_when(
      name == "confirmed" ~ value + 6e6,
      name != "confirmed" ~ value + 0.01 
    ),
    label = case_when(
      name == "confirmed" ~ comma(value, 1),
      name != "confirmed" ~ percent(value, 0.1)
    )
  ) %>%
  ggplot(aes(Country, value, fill = Country)) +
  geom_col(colour = "black") +
  geom_label(
    aes(y = lab_y, label = label), 
    alpha = 0.3
  ) +
  facet_grid(
    Continent~name, 
...
a <- topDeaths %>%
  ...
  geom_col(colour = "black") +
  geom_label(
    aes(y = rate + 0.001, label = percent(rate, 0.01)),
    fill = "white", alpha = 0.5
  ) +
  scale_fill_viridis_d(option = "magma", direction = 1) +
  scale_y_continuous(expand = expansion(c(0, 0.08)), labels = percent) +
  labs(y = "Fatalities (% Population)") +
  coord_flip() +
...
b <- topDeaths %>%
  ...
  geom_col(colour = "black") +
  geom_label(
    aes(y = deaths + 1e5, label = comma(deaths, accuracy = 1)),
    fill = "white", alpha = 0.5
  ) +
  scale_fill_viridis_d(option = "magma", direction = 1) +
...
cp <- glue(
  ...
  For fatality rates scaled by population size, it is important to realise that a value of 1000 indicates that one in every 000 people from the total population has died (ignoring demographics).
  Similarly, a value of 2000 indicates that 1 in every 500 from the total population has died in that country.
...
A very brief breakdown of US data reveals that the governing party has now had an effect on how hard a state has been impacted by COVID-19.
It is also very clear that population density is a key driving factor behind the severity of the outbreaks, as measured by the total fatality rate.
...
gov <- "https://en.wikipedia.org/wiki/List_of_current_United_States_governors" %>%
  read_html() %>%
  html_nodes("body") %>%
  xml2::xml_find_all("//table[contains(@class, 'sortable wikitable')]") %>%
  .[[1]] %>%
...
dens <- "https://en.wikipedia.org/wiki/List_of_states_and_territories_of_the_United_States_by_population_density" %>%
  read_html() %>%
  html_nodes("body") %>%
  xml2::xml_find_all("//table[contains(@class, 'wikitable sortable')]") %>%
  .[[1]] %>%
  html_table(header = FALSE, fill = TRUE) %>%
  as_tibble() %>%
  setNames(
...
df %>% 
  ...
  geom_text(aes(label = Death_Rate, colour = Party), nudge_y = 150, show.legend = FALSE) +
  geom_hline(yintercept = c(500, 1000), colour = "black", linetype = 2) + 
  coord_flip() + 
...
## Infections By Population Density


```{r plotCasesDensity, fig.height=8, fig.width=8, fig.cap = "*Cases Per Million shown as a function of Population Density. Regression lines are drawn for each governing party, with a clear difference in slopes and intercepts (see analysis below).*"}
df %>% 
  ggplot(aes(Density, Incident_Rate/10, colour = Party)) +
  geom_point() +
...
## Fatalities By Population Density
...
A Poisson regression was fitted using Generalised Linear Models for both confirmed case numbers and fatalities

```{r}
glm(Confirmed ~ Party * Density, family = "poisson", offset = log(Population), data= df) %>%
  tidy() %>%
  mutate(
  ...
  dplyr::select(
    Term = term, Estimate = estimate, p = p.value, ` `
  ) %>%
  pander(
    caption = "
    *Results fitting __confirmed cases__ as a function of governing party and population density.
    Surprisingly, a subtle decrease in case numbers was found for increasing population density, 
    with this decrease being even stronger in states with a Republican governor.
    The key interpretation is that for states with a lower population density, having a 
    Republican governor was strongly associated with a higher infection rate.*
    "
  )
glm(Deaths ~ Party * Density, family = "poisson", offset = log(Population), data = df) %>%
  tidy() %>%
  mutate(
    estimate = round(estimate, 3),
    ` ` = gtools::stars.pval(p.value),
  ...
  pander(
    caption = "
    *Results fitting __total fatalities__ as a function of governing party and population density.
    A small decrease in the number of fatalities was found for states with a Republican governor,
    however, this became an increase with increasing population density.*
    "
  )
glm(Deaths ~ Party * Density, family = "poisson", offset = log(Confirmed), data = df) %>%
  tidy() %>%
  mutate(
    estimate = round(estimate, 3),
    ` ` = gtools::stars.pval(p.value),
  ...
  pander(
    caption = "
    *Results fitting __fatality rate within confirmed cases__ as a function of governing party and population density.
    A decrease in the fatility rate was found for states with a Republican governor,
    however, this was again found to become an increase with increasing population density*
    "
  )
```
