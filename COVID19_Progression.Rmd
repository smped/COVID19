...
vicUrl <- c(
  "https://www.dhhs.vic.gov.au/coronavirus-update-victoria-{day(dt)}-{month.abb[month(dt)]}-{year(dt)}",
  "https://www.dhhs.vic.gov.au/coronavirus-update-victoria-{day(dt-1)}-{month.abb[month(dt-1)]}-{year(dt-1)}"
  ) %>% 
...
waMedia <- "https://ww2.health.wa.gov.au/News/Media-releases-listing-page" %>%
  read_html() %>% 
  html_nodes("body") %>% 
  xml_find_all("//a[contains(@title, 'COVID-19 update')]") %>%
  .[[1]] %>%
  html_attr("href")
...
waRecov <- waTxt %>%
  ...
  str_replace_all(".+ ([0-9]+) .+recovered.+", "\\1") %>%
  as.numeric()
latestAU$WA <- tibble(
...
altSaUrl <- c(
  "https://www.sahealth.sa.gov.au/wps/wcm/connect/public+content/sa+health+internet/about+us/news+and+media/all+media+releases/covid-19+update+{month.abb[month(dt)]}+{day(dt)}+{year(dt)}",
  "https://www.sahealth.sa.gov.au/wps/wcm/connect/public+content/sa+health+internet/about+us/news+and+media/all+media+releases/covid-19+update+{month.abb[month(dt-1)]}+{day(dt-1)}+{year(dt-1)}"
  ) %>% 
  vapply(glue, character(1)) %>% 
  str_to_lower()
validSa <- altSaUrl %>%
...
## Cumulative Infection Rates {.tabset}
...
```{r plotPCA, fig.cap = cp3, fig.width=7, fig.height=7}
set.seed(101)
pca$x %>%
  as.data.frame() %>%
...
### Cumulative Fatalities (Unscaled)
...
cpDeath <- glue(
  ...
  Regions are shown as defined by the UN, however some with fewer points were merged for convenience.*"
)
```


```{r plotDeath, fig.cap=cpDeath, fig.width=10, fig.height=10}
ggplotly(deathPlot)
```
...
dailyVsTotalDeaths <- deaths %>%
  ...
  coord_cartesian(ylim = c(0.05, 2e3)) + 
  facet_wrap(~Region, ncol = 3)
```
...
b <- deaths %>%
  ...
  theme(legend.position = "none")
cp <- glue(
  "*Comparison of the {nrow(a$data)} most impacted countries by fatalities per million showing A) the number of fatalities scaled by population size and B) Total Fatalities.
  ...
  Importantly, whilst the fatality count in countries like the US and the UK are far below the 'excess deaths' observed throughout these countries, this is not true for Belgium and this country is more likely to reflect an accurate assessment of the true fatalities due to COVID-19, whilst the values for the US and the UK are considerable under-estimates.
  Only countries with populations > {comma(minPop)}, and with more than {minRate} fatalities/million are shown.
...
### Active Infections Across All Countries
...
- Active infection rates in many countries are currently declining **especially in Australia and New Zealand**
- Despite strict control measures early on in the pandemic, active infection rates are still exponentially increasing in Singapore
...
```{r plotP4, fig.height=9, fig.width=8, fig.cap = glue("*Fatality, Recovery and Active Infection rates for countries which have exceeded {startingPoint} confirmed cases / million, and with a population size > {comma(minPop)}. Countries are ordered by the percentageof cases that remain active.*")}
ggplotly(p4)
```
