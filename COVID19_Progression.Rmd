...
```{r ausStatsPlot, fig.height=7, fig.width=9, fig.cap = "*Current confirmed and recovered cases, along with fatalities for Australia only. Active cases are shown as confirmed cases excluding fatalities and those classed as recovered. Loess curves through all points are shown as continuous lines.*"}
ggplotly(
  confirmed %>%
    ...
    distinct(Type, Total, .keep_all = TRUE) %>%
    mutate(
...
tested %>%
  ...
  pander(
    ...
    caption = glue(
      ...
      No testing statistics have yet been released by the NT Health department.*"
    ),
    emphasize.strong.rows = nrow(.)
  )
```
