...
plotIncVConf <- confirmed %>%
  ...
  rename(`Daily Increase` = D) %>%
  mutate(Population = comma(round(Population, -3))) %>%
  ggplot(aes(Confirmed, `Daily Increase`, colour = Country, label = Date, key = Population)) +
  geom_hline(
    aes(yintercept = `Daily Increase`),
    data = . %>%
...
```{r showIncVConf, fig.cap=capIncVConf, fig.height=7, fig.width=9}
...
- Despite the above, Australia is **still in the exponential growth phase**, however there are early signs of the growth levelling out.
...
vicTests <- NA_real_
vicUrl <- glue(
  "https://www.dhhs.vic.gov.au/coronavirus-update-victoria-{str_trim(format(Sys.Date(), '%e-%B-%Y'))}"
) %>%
  str_to_lower()
...
```{r saTests}
saTests <- NA_real_
saTests <- "https://www.sa.gov.au/covid-19/dashboard" %>%
  read_html() %>%
  xml_find_all("//div[contains(@id, 'content_container_605362')]") %>%
  html_text() %>%
  str_split("\r\n") %>%
  .[[1]] %>%
  str_trim() %>%
...
```{r oldSaUrl, echo = FALSE,eval=FALSE}
# saUrl <- paste0(
...
tested %>%
  ...
  pander(
    ...
    caption = glue(
      ...
      ^+^As no testing statistics have yet been released by the NT Health department via press releases, the number of known tests was updated manually from the audio of press conferences.*"
    ),
    emphasize.strong.rows = nrow(.)
  )
```
