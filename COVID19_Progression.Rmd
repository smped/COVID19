...
confirmed %>%
  ...
  mutate(
    ...
    occurence = Population / confirmed,
    Population = round(Population, -5) / 1e6
  ) %>%
  dplyr::filter(rate >= 1) %>%
  arrange(desc(rate)) %>%
  ...
  datatable(
    options = list(
      ...
      searchCols = list(
        ...
        list(
          search = glue(
            '{minPop/1e6} ... {max(pops$Population/1e6)}'
          )
        ),
        NULL
      )
    ...
    caption = paste(
      ...
      "Rate represent the latest confirmed infection rate as cumulative cases per million people, whilst Occurence represents the number of people expected before one case is found, assuming an even distribution amongst the population.",
      "In other words, one in every 'Occurence' people within the population have been confirmed to have contracted COVID-19.",
      "Occurence is inversely proportional to Rate.",
  ...
  formatCurrency(
    columns = c("Confirmed", "Rate (Cases per Million)", "Occurence"),
    currency = "", 
    digits = 0,
    mark = ","
  )
...
```{r plotP4, fig.height=7, fig.cap = glue("*Fatality, Recovery and Active Infection rates for countries which have exceeded {startingPoint} confirmed cases / million. Countries are shown in order of the date they passed {startingPoint} confirmed cases / million*")}
ggplotly(p4)
```
