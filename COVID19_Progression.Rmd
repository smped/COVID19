...
# THis dataset currently switches SA & TAS counts on 14/15 May
...
```{r plotly, fig.width=10, fig.height=9, fig.cap = cp}
ggplotly(
  p, 
  tooltip = c(
...
```{r showIncVConf, fig.cap=capIncVConf, fig.height=9, fig.width=10}
ggplotly(plotIncVConf + facet_wrap(~Region, ncol = 4))
```
...
```{r showScaledDeaths, fig.cap=cp, fig.width=10, fig.height=9}
scaledDeathPlot %>%
  ggplotly(
...
ggplotly(dailyVsTotalDeaths + facet_wrap(~Region))
```
...
```{r plotly2, fig.width=10, fig.height=9, fig.cap = cp2}
ggplotly(p2) 
```
