...
confirmed %>% 
  ...
  dplyr::select(-tot) %>%
  mutate(`%Increase` = percent(`%Increase` + 1)) %>%
  pander(
    justify = "lrrr",
    caption = paste(
...
tested %>%
  ...
  bind_rows(
    tibble(
      ...
      mutate(
        `Percent Tested` = `Total Tested` / Population,
        Positive = Confirmed / `Total Tested`,
        Negative = 1 - Positive
      )
  ) %>%
  datatable(
    ...
    caption = glue(
      ...
      As no testing results have yet been released by the NT Health department, figures for this Territory have been excluded."
    )
  ) %>%
  formatCurrency(
    columns = c("Population", "Confirmed", "Total Tested"),
    currency = "",
    digits = 0,
    mark = ","
  ) %>%
  formatPercentage(
    columns = c("Percent Tested", "Positive", "Negative"),
    digits = 2
  )
```
