...
vicTests <- vicTxt %>%
  str_subset("[0-9]+.*(people|samples).*test(ed|s|ing)") %>%
  .[[1]] %>%
  str_replace_all(
    ".+ ([0-9,]+) .*test(ed|s|ing).+",
    "\\1"
...
```{r}
rarelyShow <- c("Slovakia", "Paraguay", "Azerbaijan", "Liberia", "Burkina Faso", "Belarus", "Panama", "Hungary", "Bulgaria", "Tunisia", "Moldova")
```
...
cp <- glue(
  ...
  Countries are shown in order of passing {minRate} deaths / million."
)
```


```{r showScaledDeaths, fig.cap=cp, fig.width=9, fig.height=7}
scaledDeathPlot %>%
  ggplotly(
