...
This excludes countries such as `r pander(dplyr::filter(pops, Population < minPop)$Country)`.
  ...
  datatable(
    options = list(
      ...
      searchCols = list(
        ...
        list(
          search = glue(
            '{as.integer(minPop)} ... {max(pops$Population)}'
          )
        ),
        NULL
      )
    ...
    caption = paste(
      ...
      "The intial filter is set so that only countries with a population greater than", comma(minPop), "are shown.",
