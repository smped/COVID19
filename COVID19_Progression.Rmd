...
confirmed %>%
  ...
  mutate(
    ...
    rate = round(rate, 1),
    Population = round(Population, -4) / 1e6
  ) %>%
  arrange(desc(rate)) %>%
  ...
  datatable(
    ...
    caption = paste(
      ...
      "The final column provides the latest confirmed infection rate as cumulative cases per million people.",
  ...
  formatRound(
    columns = "Rate (Cases per Million)",
    digits = 1
  )
```
