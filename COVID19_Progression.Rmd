...
However, all countries are likely to contain many unreported cases given the incomplete testing regimes in place for literally every country.
Similarly, reporting in many countries may have features that cause concerns regarding data integrity and this makes comparison across contries difficult.
...
cp <- paste(
    ...
    "The flatter the line, the slower the growth and a prefectly horizontal line would indicate zero growth, or no new confirmed cases.*"
)
```

```{r plotly, fig.width=10, fig.height=8, fig.cap = cp}
ggplotly(p)
```
