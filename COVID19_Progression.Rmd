...
nswTests <- NA_real_
nswUrl <- glue(
  "https://www.health.nsw.gov.au/news/Pages/{str_remove_all(dt, '-')}_00.aspx"
)
...
if (url.exists(waUrl)){
  ...
  waRecov <- waTxt %>%
    str_subset(pattern = "recovered") %>%
    str_replace_all(pattern = ".+ ([0-9]+) recovered.+", "\\1") %>%
    as.numeric()
}
```
...
saUrl <- "https://www.sa.gov.au/covid-19/dashboard"
saTxt <- saUrl  %>%
  read_html() %>%
...
```{r updateRecovered}
auRec <- data.frame(obj = ls(pattern = "Recov"), stringsAsFactors = FALSE) %>% 
  mutate(
    recovered = vapply(obj, function(x){eval(str2lang(x))}, numeric(1)),
    `Province/State` = case_when(
      str_detect(obj, "act") ~ "Australian Capital Territory",
      str_detect(obj, "nt") ~ "Northern Territory",
      str_detect(obj, "sa") ~ "South Australia",
      str_detect(obj, "wa") ~ "Western Australia",
      str_detect(obj, "nsw") ~ "New South Wales",
      str_detect(obj, "vic") ~ "Victoria",
      str_detect(obj, "qld") ~ "Queensland",
    ),
    date = max(govTable$date),
    Country = "Australia"
  ) %>%
...
confirmed %>% 
  ...
  left_join(
    latestAU %>%
      dplyr::select(`Province/State`, date, deaths)
  ) %>%
  bind_rows(
    group_by(., date) %>%
      summarise_at(vars(confirmed, deaths), sum, na.rm = TRUE) %>%
      mutate(`Province/State` = "Total")
  ) %>%
  mutate(FRate = deaths / confirmed) %>% 
  group_by(`Province/State`) %>% 
  mutate(
    d = diff(confirmed),
  ...
  pivot_wider(
    ...
    names_from = date, 
    values_from = c(confirmed, d, p, deaths, FRate)
  ) %>% 
  set_names(
    str_remove(names(.), "confirmed_")
  ) %>%
  dplyr::select(1:4, 6, ncol(.) - 2, ncol(.)) %>%
  set_names(
  ...
  set_names(
    str_replace(names(.), "deaths.+", "deaths")
  ) %>%
  set_names(
  ...
  pander(
    justify = "lrrrrrr",
    caption = paste(
      "*Confirmed cases and fatalities reported by each state at time of preparation.",
...
Most states show a strong upward curve during the week following 19^th^ March, clearly showing the impact of the Ruby Princess.
The hypothetical growth in the infection rate without that event occurring is not calculated here, however it is clear that the spread of COVID-19 has not continued at the same pace after that event initially occured.*
"
)
```
...
If no difference is noted between any state and NSW, it can be consideed that these states are all experiencing growth at a similar rate.
...
new <- latestAU %>%
    ...
    dplyr::filter(State != "Northern Territory") %>%
    droplevels()
rate2Val <- function(x){10^x / 1e6}
new %>%
  cbind(
    predict.lm(
      lm,
  ...
  mutate(`Prediction Interval` = glue("[{lwr}, {upr}]")) %>%
  dplyr::select(State, date, Latest = confirmed, `Prediction Interval`) %>%
  rename_all(str_to_title) %>%
  pander(
    justify = "llrr",
    caption = glue(
      "*95% prediction intervals for cumulative confirmed cases in each state on {format(max(.$Date), '%B %d, %Y')}.
