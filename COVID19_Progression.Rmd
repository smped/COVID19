...
confirmed %>%
  ...
  mutate(
    ...
    rate = round(rate, 1),
    Population = round(Population, -3) %>% as.integer()
  ) %>%
  arrange(desc(rate)) %>%
  ...
  datatable(
    options = list(
      ...
      searchCols = list(
        ...
        list(
          search = glue(
            '{as.integer(minPop) + 1L} ... {max(pops$Population)}'
          )
        ),
        NULL
      )
  ...
  formatCurrency(
    columns = c("Confirmed", "Population"),
    currency = "", 
    digits = 0,
    mark = ","
  ) %>%
...
confirmed %>% 
  ...
  ungroup() %>%
  mutate(tot = grepl("Total", `Province/State`)) %>%
  arrange(tot, desc(Increase)) %>%
  dplyr::select(-tot) %>%
  mutate(Increase = percent(Increase, accuracy = 0.1)) %>%
  rename(`Daily Increase` = Increase) %>%
  pander(
    justify = "lrrr",
    caption = paste(
...
```{r, fig.cap="Cumulative fatalities wthin Australia"}
