...
cp <- glue(
  ...
  Whilst still flawed this may give a less biased estimate on the true fatality rate, and will always be higher than the alternative calculation.
  The intial fatality rate spiked above 30% during the intial outbreak under this approach, and as such, data is only shown after {format(ymd(minDate), '%d %b, %Y')}.
  All times used in the offset were obtained from [here](https://wwwnc.cdc.gov/eid/article/26/6/20-0320_article)*"
)
```
