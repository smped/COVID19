...
```{r plotDaily, fig.width=10, fig.height=8, fig.cap = "Plot of daily new cases for the last year. Coloured lines represent 7 day averages."}
plotly::ggplotly(
  data %>% 
    ...
    coord_cartesian(xlim = c(dt - 365/2, dt)) +
    scale_colour_brewer(palette = "Set1") +
...
```{r plotTestPos, fig.width=10, fig.height=7, fig.cap = "Test Positivity Rate. A single click in the plot legend will show/hide that State. The 5% rate, below which the pandemic is considered manageable by the WHO is shown as a dashed line."}
plotly::ggplotly(
  data %>% 
...
```{r plotHosp, fig.width=10, fig.height=8, fig.cap = "Plot of daily new hospitalisations for the last year. Coloured lines represent 7 day averages."}
plotly::ggplotly(
  data %>% 
    dplyr::filter(REPORT_DATE <= dt, CASE_CNT > 0) %>% 
    arrange(CODE, REPORT_DATE) %>% 
...
```{r plotDeath, fig.width=10, fig.height=8, fig.cap = "Plot of daily fatalities for the last year. Coloured lines represent 7 day averages."}
plotly::ggplotly(
  data %>% 
    dplyr::filter(REPORT_DATE <= dt, CASE_CNT > 0) %>% 
    arrange(CODE, REPORT_DATE) %>% 
    mutate(
      Fatalities = DEATH_CNT - PREV_DEATH_CNT,
      Fatalities = ifelse(is.na(Fatalities), 0, Fatalities)
    ) %>%
    # dplyr::select(REPORT_DATE, CODE, DEATH_CNT, CASE_CNT) %>% 
    group_by(CODE) %>% 
    mutate(
      MA7 = sma(Fatalities, 7),
      State = case_when(
        CODE == "AUS" ~ "All States",
        TRUE ~ auStates[CODE]
      ),
    ...
    ggplot(aes(Date, colour = State)) + 
    geom_point(aes(y = Fatalities), alpha = 0.5, colour = "grey", size = 1/2) +
    geom_segment(
      aes(xend = Date, y = 0, yend = Fatalities),
      colour = "grey", alpha = 0.5
    ) +
    geom_line(aes(y = `7 Day Average`)) +
    facet_wrap(~State, scales = "free_y", ncol = 1) +
    coord_cartesian(xlim = c(dt - 365, dt)) +
    scale_colour_brewer(palette = "Set1") +
