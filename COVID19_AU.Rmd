...
```{r updateNSW}
nswTests <- NA_real_
# nswUrl <- "http://www.health.nsw.gov.au/_layouts/feed.aspx?xsl=1&web=/news&page=4ac47e14-04a9-4016-b501-65a23280e841&wp=baabf81e-a904-44f1-8d59-5f6d56519965&pageurl=/news/Pages/rss-nsw-health.aspx" %>%
nswUrl <- GET("https://www.health.nsw.gov.au/_layouts/feed.aspx?xsl=1&web=/news&page=4ac47e14-04a9-4016-b501-65a23280e841&wp=baabf81e-a904-44f1-8d59-5f6d56519965&pageurl=/news/Pages/rss-nsw-health.aspx", config = httr::config(ssl_verifypeer = FALSE)) %>%
  read_xml() %>%
  xml_find_all("//item") %>%
  .[[1]] %>%
  as.character() %>%
...
nswRecov <- "https://covidlive.com.au/report/daily-recoveries/nsw" %>% 
  read_html() %>% 
  html_nodes("body") %>% 
  xml_find_all("//table[contains(@class, 'DAILY')]") %>%
  html_table() %>%
  .[[1]] %>% 
  as_tibble() %>%
  dplyr::select(DATE, RECOV) %>%
  mutate(
    RECOV = str_remove_all(RECOV, ",") %>% 
      str_replace_all("^$", "0") %>%
      as.numeric(),
    DATE = paste(DATE, "2020") %>%
...
latestAU$NSW <- tibble(
  ...
  deaths = nswTable %>%
    ...
    .[["Count"]],
  recovered = dplyr::filter(nswRecov, DATE == dt)$RECOV,
  tested = nswTests
)
```
...
- NSW Health made a significant change to their recovered cases when **cases not tracked after 6 weeks were classified as recovered**. An additional change was subsequently made such that active cases are now formally classed as locally acquired within the last 4 weeks. Due to the resultant incompatibility of running statistics, this change has not yet been implemented below.
...
The current `r n` day growth factor is `r round(dplyr::filter(gf$data, date == max(date), confirmed == max(confirmed))$R, 2)` which gives `r ifelse(dplyr::filter(gf$data, date == max(date), confirmed == max(confirmed))$R > 1, "*considerable cause for concern.* In particular, the **outbreak in Victoria is not yet under control**", "some degree of confidence that the spread of infections is relatively under control")`.
