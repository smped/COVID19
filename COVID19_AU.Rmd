...
df <- data %>%
  ...
  dplyr::mutate(
    `Daily Change` = CASE_CNT - PREV_CASE_CNT,
  ...
  dplyr::select(
    State = CODE, CASE_CNT, `% Total`, `% Active`,
...
tbl <- df %>%
  ...
  reactable(
    ...
    columnGroups = list(
      colGroup(name = "Cumulative Cases", columns = c("CASE_CNT", "% Total")),
      colGroup(name = "Active Cases", columns = c("% Active", "ACTIVE_CNT", "Daily Change", "% Change")),
      colGroup(name = "Infection Rates", columns = c("Rate", "PPI"))
    ),
...
df <- data %>%
  ...
  dplyr::select(
    State = CODE, hospitalised = MED_HOSP_CNT,
...
tbl <- df$states %>% 
  reactable(
    columns = list(
      ...
      change_hosp = colDef(
        name = "Change",
          ...
          list(fontWeight = wt, color = color)
          },
        maxWidth = 80,
    ...
    columnGroups = list(
      colGroup(name = "Hospitalised", columns = str_subset(colnames(df$states), "(^hosp|hosp$)")),
      colGroup(name = "ICU", columns = str_subset(colnames(df$states), "ICU|icu")),
      colGroup(name = "Ventilated", str_subset(colnames(df$states), "VENT|vent"))
    ),
    defaultColDef = colDef(footerStyle = list(fontWeight = "bold"))
  )
cp <- glue(
  ...
  "The % of currently hospitlaised cases which are in ICU or being ventilated ",
  "is also given. ",
...
df <- data %>% 
  ...
  left_join(ausPops, by = "State") %>% 
  dplyr::select(CODE, State, contains("DEATH"), `Total Cases` = CASE_CNT) %>% 
  left_join(
    data %>% 
  ...
  dplyr::select(
    State = CODE, `Total Cases`, `Total Fatalities` = DEATH_CNT, contains("Fatal")
  ) %>% 
  split(.$State == "AUS") %>% 
  setNames(c("states", "national"))
tbl <- df$states %>% 
...
```{r plotActive, fig.width=10, fig.height=6, fig.cap = "Plot of active infections against time for the last year. Double-click on a state in the legend to only see that state, or single-click individually to add or remove."}
plotly::ggplotly(
  data %>% 
    dplyr::mutate(
      State = case_when(
