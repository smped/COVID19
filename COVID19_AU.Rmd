...
Using an estimated population size of `r comma(max(ausPops$Population))`, the total percentage of the Australian population *confirmed as having been infected at some point* currently sits at `r percent_format(accuracy = 0.1)(dplyr::filter(data, REPORT_DATE == dt, CODE == "AUS")$CASE_CNT / max(ausPops$Population))`, or one person in every `r round(max(ausPops$Population) / dplyr::filter(data, REPORT_DATE == dt, CODE == "AUS")$CASE_CNT, 1)`. 
  ...
  reactable(
    columns = list(
      ...
      `Fatality Rate` = colDef(
        ...
        footer = percent(df$national[["Fatality Rate"]], 0.01)
      )
    ),
...
## Cumulative Fatalities: Population Scaled

```{r plot-annual, fig.height=8, fig.width=10, fig.cap = "State-level Fatalities scaled by estimated population, starting at 1-July-2021 as an approximate start date for the Australian pandemic. A value of 500 on the y-axis indicates that 1 in every 2000 people has died."}
ggplotly(
  weekly_df %>% 
    ...
    mutate(
      ...
      `People Per Death` = round(Population / Total, 0),
      Total = comma(Total)
    ) %>% 
    ungroup() %>% 
    mutate(State = factor(State, levels = ausPops$State)) %>% 
    ggplot(
      aes(Date, Rate, colour = State, label = Total, key =`People Per Death`)
    ) +
    geom_line() +
    scale_colour_manual(
      values = c(hcl.colors(length(ausPops$State) - 1, "Dark 3"), "black")
  ...
  tooltip = c("State", "Date", "Rate", "Total", "People Per Death")
)
```
