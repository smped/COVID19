...
data %>% 
  ...
  group_by(CODE) %>% 
  mutate(
    weekly_change = NEW_CASE_CNT / lag(NEW_CASE_CNT, 7),
    MA7 = sma(weekly_change, 7)
  ) %>% 
  ungroup() %>%
  dplyr::filter(REPORT_DATE > dt - 100) %>% 
  ggplot(
    aes(REPORT_DATE, weekly_change, colour = CODE)
  ) +
  geom_line() +
  geom_line(aes(y = MA7), colour = "grey30", linetype = 2) +
...
```{r, fig.width=10, fig.height=10, fig.cap = "Hospitalisation rate using the 7-day total number of new cases to represent active cases, and projecting hospitalisations back onto the total active case estimates from 1 week prior."}
data %>% 
  dplyr::select(
  ...
  dplyr::filter(date > today() - 135) %>% 
  ggplot(aes(date, p_hosp, colour = State)) +
  geom_line()  +
  facet_wrap(~State, scales = "free_y") +
