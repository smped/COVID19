...
```{r plot-annual, fig.height=8, fig.width=10, fig.cap = "State-level Fatalities for 2020 scaled by estimated population."}
ggplotly(
  weekly_df %>% 
    left_join(ausPops, by = "State") %>% 
    dplyr::filter(year >= 2022) %>% #, CODE != "AUS") %>% 
    group_by(State) %>% 
    mutate(
      Total = cumsum(Total),
    ...
    labs(x = "Date", y = "Total Deaths / Million (2020 Only)"),
